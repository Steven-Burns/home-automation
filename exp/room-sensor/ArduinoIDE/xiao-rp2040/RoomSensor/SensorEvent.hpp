...
struct KeyPressEvent : public SensorEvent
{
  ...
  virtual const char* ToString() const 
  {
    ...
    sprintf(toStringBuffer, "%s k=%d press=%s", tag, keyNumber,
      kind == KeyState::KeyPressKind::DoublePress ? "dbl" : 
        kind == KeyState::KeyPressKind::SinglePress ? "sing" : 
          kind == KeyState::KeyPressKind::LongPress ? "long" : "unk");
    return toStringBuffer;
  }
};
...
/*
struct LightLevelEvent : public SensorEvent("LITE")
{
